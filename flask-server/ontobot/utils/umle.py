...
class UMLE:
    ...
    def check_category_level(self):
        for concept in self.__set_difference:
            ...
            if err_class['level'] > 0 and err_class['stereotype'] == 'category':
                err_cls = self.Err()
                err_cls.concept_name = concept
                err_cls.stereotype = err_class['stereotype']
                err_cls.suggestion = [f"This class/concept may not be category since super-concept of {concept} is not category"]
    ...
    def check_subkind_level(self):
        for concept in self.__set_difference:
            ...
            if err_class['level'] == 0 and err_class['stereotype'] == 'kind':
                ...
                err_cls.concept_name = concept
                err_cls.stereotype = err_class['stereotype']
                err_cls.suggestion = ["Since this concept is a Super-concept and it has Sub-concepts, there are many suggestions according to the ontoUML. They are,",
                "please check whether you have correctly define the disjoint property(ie: If sub-concepts are in phase, please make all of them disjoint)",
                "Change all the Sub-consepts into subkind since Super-concept is in kind format","If you don't want to disjoint the Sub-concepts you can define them as role format",
                "If there are disjoint Sub-consepts, please check wether all of them are subkind or phase. If not do changes since disjoint set should be in either subkind format or phase format"]
            ...
            if err_class['level'] > 0 and err_class['stereotype'] == 'subkind':
                ...
                if super_cls['stereotype'] == 'phase':
                    ...
                    "If there are disjoint sibling concepts with same stereotype, you can change the {concept} concept's stereotype into stereotype of those concepts and make disjoint with them.",
            ...
            if err_class['level'] > 0 and err_class['stereotype'] == 'role':
                ...
                if super_cls['stereotype'] == 'rolemixin':
                    ...
                    err_cls.suggestion = ["Since super-concept of the concept is rolemixin, there should be multiple role sub-concepts with disjoint-complete",
                        "If there is no multiple sub-concepts, this single sub-concepts should be rolemixin"]
                    ...
                    err_cls.concept_name = concept
                    err_cls.stereotype = err_class['stereotype']
                    err_cls.suggestion = ["Since super-concept of the concept is not rolemixin, the stereotype of the super-concept should be in sortal expression (kind,collective,phase,role)"]
