...
const TaxonomyBranch = ({ taxonomy, taxonomyStyle='taxonomy-name' }) => {
    ...
    useEffect(() => {
        ...
        const findAvailableSubclsses = (taxonomyObj) => {
            if (taxonomyObj.id === taxonomy.id) {
                ...
                if (taxonomyObj.hasOwnProperty('subclasses')) {
                    taxonomyObj.subclasses?.map(subCls => findAvailableSubclsses(subCls))
                }
            }
        }
    ...
    const handleSaveClass = () => {
        if (newClass.name?.length === 0) {
            ...
            let nameDuplication = availableSubclasses.filter(subCls => subCls.name === newClass.name)
    ...
    return (
        <div key={taxonomy.id} className="text-fontcolor">
            <div className="flex mt-1 text-fontcolor tracking-wider">
                ...
                <p 
                    className={taxonomyStyle} 
                    onClick={hanldeBranchVisibility}
                >
                    {taxonomy.name}
                </p>
                {
            ...
            { branchVisiblity && taxonomy.subclasses?.map((subCls) => (
            ...
            <Modal open={isModalOpen} onClose={() => setisModalOpen(false)}>
                <p className="modal_title">
                    Add subclasses to <span className="font-bold text-secondary">{taxonomy.name}</span> class.
                </p>
                                    ...
                                    <div className="items-holder rounded-md bg-white absolute top-[100%] mt-2 border w-full">
                                        {
                                            isDropdownVisible && stereotypes.map((item) =>(
                                                ...
                                                >
                                                    {
                                                        item.id !== selectedStereotype.id && <p>{item.name}</p>
                                                    }
                                                </div>
                                            ))
                                        }
