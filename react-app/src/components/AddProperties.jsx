...
const AddProperties = ({ selectedTaxonomy }) => {
    ...
    const handleModalOpening = () => {
        if (selectedTaxonomy.name === '' || selectedTaxonomy.name === 'taxonomies') {
    ...
    const handleAddProperty = (event) => {
        ...
        if (newProperty.name === '' || newProperty.datatype === '') {
            setalertMsg('Please note that Porperty name and Data type are required fields.')
        ...
        <div>
            ...
            >
                <p className="modal_title">
                    Add properties of <span className="font-bold text-secondary">{selectedTaxonomy.name}</span> class.
                </p>
                    ...
                    <div className="flex flex-col gap-2">
                        ...
                        <label className="inline-flex relative items-center mr-5 cursor-pointer">
                            <input
                                ...
                                checked={enabled}
                                onClick={() => newProperty.functional === 'yes' ? setnewProperty({...newProperty, functional: 'no'}) : setnewProperty({...newProperty, functional: 'yes'})}
                                onChange={() => setEnabled(!enabled)}
                            />
                            <div
                                className="w-11 h-6 bg-lightgray rounded-full peer  peer-focus:ring-secondary  peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-0.5 after:left-[2px] after:bg-white after:border-lightgray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-secondary"
                            />
                ...
                <table className="mt-6 w-full text-fontcolor">
                    ...
                    <tbody className="text-sm overflow-y-scroll" >
                        { propertiesList?.map((property, index) => (
                            <tr className="" key={index}>
                                <td className="pl-2">{property.name}</td>
                                <td className="pl-2">{property.datatype}</td>
                                <td className="pl-2">{property.restrictions}</td>
                                <td className="pl-2">{property.functional}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
